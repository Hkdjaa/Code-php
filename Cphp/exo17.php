...
function affichertab($tab) {
    echo '<table style="color: #d66060;" border="2">';
    echo '<tr>';

    if (isset($tab[key($tab)])) {
        foreach ($tab[key($tab)] as $cle => $valeur) {
        ...
        foreach ($tab as $ligne) {
            ...
            foreach ($ligne as $valeur) {
                echo '<td>' . htmlspecialchars(strval($valeur)) . '</td>';
            }
            echo '</tr>';
        }
    } else {
        echo '</tr></table>';
        echo 'Le tableau ne contient pas la clé "Nom".';
    }
