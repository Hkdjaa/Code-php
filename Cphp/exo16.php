...
<header>
    ...
    <form action="" method="post">
        <label> Entrez l'angle </label>
        <div class="col-sm-3">
            <input type="text" id="angle" class="form-control" name="angle" placeholder="Entrez l'angle">
        </div>
        <br>
        <br>
        <label> Entrez l'unité </label>
        <div class="col-sm-3">
            <input type="text" id="unite" class="form-control" name="unite" placeholder="Entrez l'unité">
        </div>
        <br>
        <br>
        <div class="col-sm-4">
            <button type="submit" class="form-control" id="calcul"> Convertir</button>
            <br> <br>
        </div>
    </form>
    ...
    <span style="color: #d66060;">
        ...
        function sinus($angle, $unite) {
            if ($unite === 'd') {
                $radian = deg2rad($angle);
            } elseif ($unite === 'g') {
                $radian = ($angle / 200) * M_PI;
            } elseif ($unite === 'r'){
                $radian = $angle;
            }
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $unite = isset($_POST["unite"]) ? strtolower($_POST["unite"]) : 'r';
            echo "Voici le sinus : " . sinus($angle, $unite);
            echo " en $unite";
        }
        ?>
    </span>
