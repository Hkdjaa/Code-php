...
<header>
    ...
    <main>
        <form action="" method="get">
            <br>
            <br>
            <label style="color: #d66060;"> Bienvenue dans ce jeu de tirage. Vous devez appuyer sur "faire le tirage" et ensuite voir les valeurs qui ont été tirées; il vous faudra continuer jusqu'à obtenir e nombre mystère souhaité.</label>
            <br>
            <br>
            <label style="color: #d66060;"> Bonne chance haha ! </label>
            <br>
            <br>
            <div class="col-sm-3">
        <input type="submit" value="Effectuer le tirage" name="tirage" class="form-control" id="calcul">
</div>
        <br>
        <br>
        </form>
...
if (isset($_GET['tirage'])) {
    do {
        $tirage = random_int(100, 200);
        echo "Résultat du tirage : $tirage";
        echo "<br>";

        if ($tirage != $nombre) {
            echo "$tirage n'est pas le bon nombre. ";
            echo "<br>";
            $a++; 
        } else {
            echo "Le bon nombre $tirage a enfin été tiré";
            echo "<br>";
        }
    } while ($tirage != $nombre);

    echo "<br>";
    echo "Il aura fallu $a tirages pour trouver le bon nombre. ";
}
?>
